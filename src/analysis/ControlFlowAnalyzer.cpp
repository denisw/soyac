...
PassResult*
ControlFlowAnalyzer::analyze(Module* m)
{
...
void
ControlFlowAnalyzer::reportUnused()
{
...
void*
ControlFlowAnalyzer::visitBlock(Block* b)
{
...
void*
ControlFlowAnalyzer::visitDeclarationStatement(DeclarationStatement* stmt)
{
...
void*
ControlFlowAnalyzer::visitIfStatement(IfStatement* stmt)
{
...
void*
ControlFlowAnalyzer::visitReturnStatement(ReturnStatement* stmt)
{
...
void*
ControlFlowAnalyzer::visitConstructor(Constructor* cons)
{
    if (cons->body() != NULL)
    {
...
void*
ControlFlowAnalyzer::visitFunction(Function* func)
{
    if (func->body() != NULL)
    {
...
void*
ControlFlowAnalyzer::visitPropertyGetAccessor(PropertyGetAccessor* acc)
{
    if (acc->body() != NULL)
    {
        ...
        if (!alwaysReturning)
        {
            mRBuilder->addError(
              acc,
              boost::format(
                "Get accessor of property '%1%' might not return a value "
                "in all cases.")
                 % acc->parent());
        }
...
void*
ControlFlowAnalyzer::visitPropertySetAccessor(PropertySetAccessor* acc)
{
    if (acc->body() != NULL)
    {
        acc->body()->visit(this);
...
void*
ControlFlowAnalyzer::visitFunctionParameterExpression(
...
void*
ControlFlowAnalyzer::visitVariableExpression(VariableExpression* expr)
{
    mUnused.remove(expr->target());
    return NULL;
}
