...
const std::string& Lexer::fileName() const { return mFileName; }
     ...
     */
         ...
         */
            ...
            if (ret == -1) {
                ...
                addError(str(
                    boost::format("Unexpected character '%1%'.") % mLastChar));
...
void Lexer::addError(const std::string& description)
{
    soyac::ast::Location location(
        fileName(), mCurrentLine, mCurrentColumn, mCurrentLine, mCurrentColumn);
...
const char* Lexer::lookAhead(unsigned int numChars)
{
    assert(numChars > 0);
...
void Lexer::bufferChars(int numChars)
{
...
int Lexer::readIdentifierOrKeyword(SoyaParser::semantic_type* yylval)
{
    ...
    /*
     * Otherwise, look up in the keyword map if the identifier-like string
     * we read is a keyword, and return the arppropiate token ID in this
     * case. Otherwise, it is *really* an identifier, and we store it as
...
int Lexer::readIntegerOrFloatingPointLiteral(SoyaParser::semantic_type* yylval)
{
    bool negative = false;
...
int Lexer::readDecimalIntegerLiteral(
...
int Lexer::readBinaryIntegerLiteral(
    SoyaParser::semantic_type* yylval, bool negative)
{
    std::stringstream literal;
...
int Lexer::readOctalIntegerLiteral(
    SoyaParser::semantic_type* yylval, bool negative)
{
    std::stringstream literal;
...
int Lexer::readHexadecimalIntegerLiteral(
    SoyaParser::semantic_type* yylval, bool negative)
{
    std::stringstream literal;
...
void Lexer::setSemanticValueFromIntegerLiteral(
    SoyaParser::semantic_type* yylval, const std::string& literal, int base)
{
    int64_t data;
    bool isSigned;
    ...
    if (errno == ERANGE) {
        ...
        if (errno == ERANGE) {
            addError(str(
                boost::format("The integer literal '%1%' is too big for any of "
...
int Lexer::readFloatingPointLiteral(
    SoyaParser::semantic_type* yylval, bool negative)
{
    std::stringstream literal;
    bool dotRead = false;
    ...
    yylval->fpLiteral = new FloatingPointValue(
        std::strtod(literal.str().c_str(), 0), singlePrecision);

    if (errno == ERANGE) {
        addError(str(
            boost::format("The floating-point literal '%1%' is too big for any "
...
int Lexer::readCharacterLiteral(SoyaParser::semantic_type* yylval)
{
    /*
...
const char* Lexer::readEscapeChar()
{
    ...
    default:
        ret[0] = '\0';
        addError(str(boost::format("Invalid escape sequence '\\%1%'.") % c));
    }
...
int Lexer::readPunctatorOrOperator()
{
    const char* c = lookAhead();
